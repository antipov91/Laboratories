...
namespace SharpCircuit {
	...
	public class Voltage : CircuitElement {
        ...
        public double rsmVoltage
        {
            get
            {
                if (waveform == WaveType.AC)
                    return maxVoltage / Math.Sqrt(2);
                else
                    throw new ArgumentException("Wavetype is not AC");
            }
            set
            {
                if (waveform == WaveType.AC)
                    maxVoltage = value * Math.Sqrt(2);
                else
                    throw new ArgumentException("Wavetype is not AC");
            }
        }
